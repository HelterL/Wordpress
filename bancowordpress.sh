...
CREATE USER `wp_admin`@`$IP_PRIVATE_WEB` IDENTIFIED BY 'root';
GRANT ALL ON wordpress.* TO `wp_admin`@`$IP_PRIVATE_WEB`;
GRANT ALL ON *.* TO 'root'@'$IP_PRIVATE_WEB' IDENTIFIED BY 'root' WITH GRANT OPTION;
FLUSH PRIVILEGES;
EOF
